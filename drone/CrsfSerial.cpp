...
void CrsfSerial::handleSerialIn()
{
    ...
    {
        uint8_t b = uart_getc(_port);
        _lastReceive = to_ms_since_boot(get_absolute_time());
...
void CrsfSerial::checkPacketTimeout()
{
    ...
    if (_rxBufPos > 0 && to_ms_since_boot(get_absolute_time()) - _lastReceive > CRSF_PACKET_TIMEOUT_MS)
        while (_rxBufPos)
            shiftRxBuffer(1);
}
...
{
    if (_linkIsUp && to_ms_since_boot(get_absolute_time()) - _lastChannelsPacket > CRSF_FAILSAFE_STAGE1_MS)
    {
        if (onLinkDown)
            onLinkDown();
        _linkIsUp = false;
...
void CrsfSerial::packetChannelsPacked(const crsf_header_t *p)
{
    ...
    if (!_linkIsUp && onLinkUp)
        onLinkUp();
    _linkIsUp = true;
    _lastChannelsPacket = to_ms_since_boot(get_absolute_time());
