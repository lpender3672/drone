 ...
 */
class AltitudeHoldBlock : public TypedBlock<shared::NavigationState, Scalar> {
public:
    struct Params {
    ...
    AltitudeHoldBlock(const std::string& name, uint32_t update_period_us = 1000)
        : TypedBlock(name, "state", "thrust", update_period_us) {}
    ...
    bool update(uint64_t current_time_us) override {
        ...
        const double dt_s = get_dt_us(current_time_us) * 1e-6;
        mark_updated(current_time_us);

        const auto& state = input_.value;
        ...
        output_.value = Scalar(thrust, current_time_us);
        last_thrust_  = thrust;
        return true;
    }
