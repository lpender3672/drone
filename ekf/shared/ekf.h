...
template<typename Scalar, int DIM_NOMINAL, int DIM_ERROR, int DIM_NOISE>
class EKF {
public:
    ...
    // Common small Scalar-parameterised types (so derived classes can write
    // `Vec3`, `Mat3` etc. instead of `Eigen::Matrix<Scalar, 3, 1>`).
    using Vec3 = Eigen::Matrix<Scalar, 3, 1>;
    using Vec4 = Eigen::Matrix<Scalar, 4, 1>;
    using Mat3 = Eigen::Matrix<Scalar, 3, 3>;
    using Quat = Eigen::Quaternion<Scalar>;

    // State / covariance types
    using NominalVector = Eigen::Matrix<Scalar, DIM_NOMINAL, 1>;
    using ErrorVector = Eigen::Matrix<Scalar, DIM_ERROR, 1>;
    using CovMatrix = Eigen::Matrix<Scalar, DIM_ERROR, DIM_ERROR>;
    using NoiseMatrix = Eigen::Matrix<Scalar, DIM_NOISE, DIM_NOISE>;
    using ProcessNoiseMap = Eigen::Matrix<Scalar, DIM_ERROR, DIM_NOISE>;

    template<int M>
    using MeasVector = Eigen::Matrix<Scalar, M, 1>;
    template<int M>
    using MeasMatrix = Eigen::Matrix<Scalar, M, DIM_ERROR>;
    template<int M>
    using MeasCov = Eigen::Matrix<Scalar, M, M>;

    virtual ~EKF() = default;

    explicit EKF(const EkfErrorParameters&)
    { }
    ...
    EkfStatus getStatus(double max_variance_threshold) const
    {
        ...
        for (int i = 0; i < DIM_ERROR; i++) {
            double diag = static_cast<double>(P_(i, i));
            ...
            for (int j = 0; j < DIM_ERROR; j++) {
                if (i == j) continue;
                off_diag_sum += std::abs(static_cast<double>(P_(i, j)));

                if (std::abs(static_cast<double>(P_(i, j) - P_(j, i))) > SYM_TOL) {
...
protected:
    ...
    Scalar scratch_[SCRATCH_SIZE];
    BufferReserverT<Scalar> reserver_{scratch_, SCRATCH_SIZE};
    ...
    void update_internal(
        ...
        const MeasCov<M>& R)
    {
        auto PHt = reserver_.template matrix<DIM_ERROR, M>();
        PHt.noalias() = P_ * H.transpose();

        auto S = reserver_.template matrix<M, M>();
        S.noalias() = H * PHt + R;

        auto K = reserver_.template matrix<DIM_ERROR, M>();
        K.noalias() = PHt * S.inverse();

        auto dx = reserver_.template vector<DIM_ERROR>();
        dx.noalias() = K * z;

        // Joseph form
        auto I_KH = reserver_.template matrix<DIM_ERROR, DIM_ERROR>();
        I_KH.setIdentity();
        I_KH.noalias() -= K * H;

        auto I_KH_P = reserver_.template matrix<DIM_ERROR, DIM_ERROR>();
        I_KH_P.noalias() = I_KH * P_;

        auto KRKt = reserver_.template matrix<DIM_ERROR, DIM_ERROR>();
    ...
    virtual void inject_error(const ErrorVector&)
    { }
