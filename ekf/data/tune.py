...
with open(output_file, 'w') as f:
    ...
    f.write(f"    constexpr double SAMPLING_FREQ = {Fs:.6f};  // Hz\n\n")
    
    for axis_name, params in error_params.items():
        clean_name = axis_name.upper()
        f.write(f"    // {axis_name} parameters\n")
        f.write(f"    constexpr double {clean_name}_N = {params['N']:.10e};   // {params['N_unit']}\n")
        f.write(f"    constexpr double {clean_name}_B = {params['B']:.10e};   // {params['B_unit']}\n")
        f.write(f"    constexpr double {clean_name}_TP = {params['Tp']:.6f};  // s\n\n")
